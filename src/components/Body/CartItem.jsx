...
const CartItem = ({ post, itemIndex }) => {
  ...
  return (
    <div className="flex justify-between items-center gap-14 md:px-9 md:py-6 border-black border-b-2 text-slate-600">
      <img src={post.image} className="md:w-[165px] sm:w-[100px] w-[60px] transition-all duration-500" />

      <div className="flex flex-col md:gap-5 w-full">
        <p className="font-semibold sm:text-xl">{post.title}</p>
        <p className="md:text-base sm:text-[13px] text-[10px] ">{post.description}</p>
        <div className="flex justify-between items-center">
          <p className="text-green-600 font-bold sm:text-lg text-base">${post.price}</p>
